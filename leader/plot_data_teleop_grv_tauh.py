...
with open(file_path, 'r') as file:
    ...
    for i, line in enumerate(file):
        if i >= 29*frq and i <= 33*frq:
            ...
            jp_current_2.append(data[4])  # 2nd joint (index 1)
            jp_desired_2.append(data[1])  # 2nd joint (index 1)
            jp_current_4.append(data[6])  # 4th joint (index 3)
            jp_desired_4.append(data[3])  # 4th joint (index 3)
            jt_sum_2.append(data[7])
            jt_sum_4.append(data[9])
            gravity_2.append(data[10])
            gravity_4.append(data[12])
            # control_jt_2.append(data[30])
            # control_jt_4.append(data[32])
            # dynamics_feed_fwd_2.append(data[13])
            # dynamics_feed_fwd_4.append(data[15])
            jv_desired_2.append(data[13])
            jv_desired_4.append(data[15])
            # ja_desired_2.append(data[19])
            # ja_desired_4.append(data[21])
            invdyn_2.append(data[16])
            invdyn_4.append(data[18])
            applied_tauh_2.append(data[19])
            applied_tauh_4.append(data[21])
...
print("torque 2:", np.mean(np.abs(jt_sum_2)))
print("torque 4:", np.mean(np.abs(jt_sum_4)))

invdyn_2 = np.array(invdyn_2)
invdyn_4 = np.array(invdyn_4)
tauh_2 = jt_sum_2 - gravity_2 - invdyn_2
tauh_4 = jt_sum_4 - gravity_4 - invdyn_4
print("Tauh 2: ", np.mean(np.abs(tauh_2)))
print("Tauh 4: ", np.mean(np.abs(tauh_4)))

applied_tauh_2 = np.array(applied_tauh_2)
applied_tauh_4 = np.array(applied_tauh_4)
...
plt.plot(time_log, tauh_2, label='Tauh 2th Joint')
plt.xlabel('Time')
plt.ylabel('Tauh 2th Joint')
plt.legend()

plt.subplot(num, 2, 14)
plt.plot(time_log, tauh_4, label='Tauh 4th Joint')
plt.xlabel('Time')
plt.ylabel('Tauh 4th Joint')
plt.legend()

plt.subplot(num, 2, 15)
plt.plot(time_log, applied_tauh_2, label='Aplied Tauh 2th Joint')
plt.xlabel('Time')
plt.ylabel('Applied Tauh 2th Joint')
plt.legend()

plt.subplot(num, 2, 16)
plt.plot(time_log, applied_tauh_4, label='Applied Tauh 4th Joint')
plt.xlabel('Time')
plt.ylabel('Applied Tauh 4th Joint')
plt.legend()
