...
int openSocket(const char *remoteHost, int port = 3333) {
    ...
    if (err == -1) {
        (logMessage("openSocket(): Failed.  %s: Could not bind to socket on port %d.") % __func__ % port)
    ...
    if (err) {
        (logMessage("openSocket(): Failed.  %s: Bad IP argument '%s'.") % __func__ % remoteHost)
            .raise<std::runtime_error>();
    }
    ...
    if (err) {
        (logMessage("openSocket(): Failed.  %s: Could not set datagram destination.") % __func__)
            .raise<std::runtime_error>();
    }
...
void ghcEntryPoint(GimbalsHandController *ghc, const char *remoteHost) {
    ...
    while (!boost::this_thread::interruption_requested()) {
        ...
        data = (ghc->getPointerOpen() << 0) | (ghc->getPointerClose() << 1) | (ghc->getMiddleOpen() << 2) |
               (ghc->getMiddleClose() << 3) | (ghc->getThumbOpen() << 4) | (ghc->getThumbClose() << 5) |
               (ghc->getRockerUp() << 6) | (ghc->getRockerDown() << 7);
        send(sock, &data, SIZE_OF_MSG, 0);

        if (data != data_1) {
            printf("%d,%d  %d,%d  %d,%d  %d,%d  %f\n", ghc->getThumbOpen(), ghc->getThumbClose(), ghc->getPointerOpen(),
                   ghc->getPointerClose(), ghc->getMiddleOpen(), ghc->getMiddleClose(), ghc->getRockerUp(),
